#' mctapply
#' Parallel version of tapply(). Based on the original tapply(), using
#' mclapply() instead of lapply().
mctapply <- function (X, INDEX, FUN = NULL, ..., default = NA, simplify = TRUE)
{
    FUN <- if (!is.null(FUN))
        match.fun(FUN)
    if (!is.list(INDEX))
    ...
    ans <- mclapply(X = ans[index], FUN = FUN, ...)
